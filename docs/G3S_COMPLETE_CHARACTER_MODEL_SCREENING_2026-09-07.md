...
Status: **CANONICAL / RAW-VIDEO MOTION CONTRACT LOCKED / WAN W0 PASS_BASELINE / W1 PAINTERLY DIRECTION APPROVED / W1A STRUCTURAL 1.5 BRANCH RETAINED / W1F WHOLE-FRAME LETTERBOX FAILED CROP / W1G SUBJECT FRAMING ACTIVE / SCAIL-2 NEXT ONLY IF WAN EXHAUSTS**
...
A model reaches `EXHAUSTED_FAIL` only after finite controlled tests. Runtime/loader/preprocessor failures are infrastructure failures, not model-quality evidence. Change one high-leverage variable at a time with fixed seed/input. No seed fishing and no manual rescue.
...
Automatic preprocessing v1:

- analyzes first 37 source frames with temporal-activity segmentation;
- derives a global moving-subject envelope;
- expands it for static body/head/feet margin;
- applies one fixed affine transform to the entire clip, avoiding per-frame camera breathing;
- target subject-envelope height ratio `0.48` on `640×800`;
- target center x `300`;
- target bottom y `620`;
- pre-inference margin guard for top/bottom/sides;
- explicit guard for the central square used by `CLIPVisionEncode(crop="center")`;
- uses existing `cv2` + `numpy`; no detector checkpoint/model download.
...
- W1G subject framing + ref 1.5 — **CURRENT**.
