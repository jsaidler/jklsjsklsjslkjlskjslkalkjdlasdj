...
if (-not (Test-Path $probeScript -PathType Leaf)) { Fail "Probe script not found: $probeScript. Run git pull --ff-only first." }

$os = Get-CimInstance Win32_OperatingSystem
Write-Host ("[INFO] OS: {0} {1}" -f $os.Caption, $os.Version)
if ($os.Caption -notmatch 'Windows 11') { Fail 'Windows 11 was not detected.' }

$driveName = ([System.IO.Path]::GetPathRoot($Workspace)).TrimEnd('\').TrimEnd(':')
$drive = Get-PSDrive -Name $driveName -ErrorAction SilentlyContinue
if (-not $drive) { Fail "Workspace drive not available: $driveName`:" }
$freeGB = [math]::Round($drive.Free / 1GB, 2)
Write-Host ("[INFO] Free space on {0}: {1} GB" -f $driveName, $freeGB)
if ($freeGB -lt 5) { Fail "Less than 5 GB free on $driveName`:" }

$blender = Find-BlenderExe
if (-not $blender) {
    if ($SkipInstall) { Fail 'Blender is not installed/found and -SkipInstall was requested.' }
    $winget = Get-Command winget.exe -ErrorAction SilentlyContinue
    if (-not $winget) { Fail 'Blender is not installed and winget.exe is unavailable.' }
    ...
    if ($LASTEXITCODE -ne 0) { Fail "winget Blender install failed with exit code $LASTEXITCODE" }
    $blender = Find-BlenderExe
    if (-not $blender) { Fail 'Blender installation completed but blender.exe still could not be located.' }
}

Write-Host "[OK] Blender: $blender" -ForegroundColor Green
$versionOutput = & $blender --version 2>&1
if ($LASTEXITCODE -ne 0) { Fail 'blender.exe --version failed.' }
...
# Use Start-Process with ONE explicitly quoted argument string. This avoids both
# PowerShell 5.1 NativeCommandError wrapping of stderr and the old path-with-spaces
# splitting bug caused by passing an ArgumentList array.
$argumentString = '--background --factory-startup --python-exit-code 1 --python "{0}" -- --output-dir "{1}"' -f $probeScript, $g0Dir
...
if ($proc.ExitCode -ne 0) { Fail "Blender headless process exited with code $($proc.ExitCode). See $stdout and $stderr" }
...
foreach ($required in @($png,$blend,$manifestPath)) {
    if (-not (Test-Path $required -PathType Leaf)) { Fail "Expected output missing: $required" }
}

$manifest = Get-Content $manifestPath -Raw | ConvertFrom-Json
if ($manifest.gate -ne 'G0' -or $manifest.status -ne 'PASS') { Fail 'Manifest did not report G0/PASS.' }
...
if ($actualHash -ne $manifestHash) { Fail "PNG hash mismatch: manifest=$manifestHash actual=$actualHash" }

$result = [ordered]@{
    gate = 'G0'; status = 'PASS'; timestamp = (Get-Date).ToString('o'); repo_root = $RepoRoot; workspace = $Workspace;
    blender_exe = $blender; blender_version = $manifest.blender_version; render_engine = $manifest.render_engine;
    png = $png; png_sha256 = $actualHash; blend = $blend; manifest = $manifestPath; stdout_log = $stdout; stderr_log = $stderr
}
