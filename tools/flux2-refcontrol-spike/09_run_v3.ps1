...
$PoseDir = Join-Path $InputDir 'refcontrol_poses_v3'
$SpecDir = Join-Path $Workspace 'pose_specs_v3'
$ManifestInput = Join-Path $Workspace 'input_manifest_v3.json'
$LogDir = Join-Path $Workspace 'logs'
$RunDir = Join-Path $Workspace 'run_v3'
$RunManifestPath = Join-Path $RunDir 'step8b_v3_run_manifest.json'
$RunStartedPath = Join-Path $RunDir 'step8b_v3_started.json'
$StdoutLog = Join-Path $LogDir 'step8b_v3_comfy_stdout.log'
$StderrLog = Join-Path $LogDir 'step8b_v3_comfy_stderr.log'
$ApiRoot = "http://127.0.0.1:$Port"
$OutputRoot = Join-Path $ComfyRoot 'output'
$V3OutputDir = Join-Path $OutputRoot 'flux2_refcontrol_v3'
$ClientId = 'flux2-refcontrol-v3-20260904'
...
# Keep the V2 correction prompt unchanged so V3 isolates the effect of
# genuinely different screen-space gait geometry.
$BasePrompt = @'
...
$PoseSuffix = @{
    'pose_00_contact_L_v3' = 'Contact phase: anatomical left leg is visibly forward toward screen-right and contacting the ground; right leg visibly trails. Follow the exact asymmetric screen-space geometry from image 1.'
    'pose_01_passing_L_v3' = 'Passing phase: right leg is the straight planted support leg; anatomical left knee and foot are visibly lifted and passing forward. Follow the exact asymmetric screen-space geometry from image 1.'
    'pose_02_contact_R_v3' = 'Opposite contact phase: anatomical right leg is visibly forward toward screen-right and contacting the ground; left leg visibly trails. Follow the exact asymmetric screen-space geometry from image 1.'
    'pose_03_passing_R_v3' = 'Opposite passing phase: left leg is the straight planted support leg; anatomical right knee and foot are visibly lifted and passing forward. Follow the exact asymmetric screen-space geometry from image 1.'
}
...
Write-Host ''
Write-Host 'FLUX.2 Klein + RefControl Pose - STEP 8B: V3 DISTINCT-GAIT ONE-SHOT RUN' -ForegroundColor Cyan
Write-Host 'Goal: test whether genuinely different screen-space skeleton geometry restores L/R gait alternation.' -ForegroundColor Yellow
Write-Host 'Unchanged from V2: model, reference, prompt contract, seed, 768x1024, 20 steps, CFG 5.0, LoRA 1.0, Euler.' -ForegroundColor Yellow
Write-Host 'Changed from V2: V3 skeleton geometry only.' -ForegroundColor Yellow
Write-Host "Workspace: $Workspace" -ForegroundColor DarkGray
Write-Host ''

if ($Seed -ne 20260904) { throw "V3 contract requires seed 20260904. Got $Seed." }
if ($Width -ne 768 -or $Height -ne 1024) { throw "V3 contract requires 768x1024. Got ${Width}x${Height}." }
if ($Steps -ne 20) { throw "V3 contract requires 20 steps. Got $Steps." }
if ([math]::Abs($Cfg - 5.0) -gt 0.0001) { throw "V3 contract requires CFG 5.0. Got $Cfg." }
...
if ([string]$inputManifestObject.revision -ne 'v3_walk_distinct_screenspace') { throw 'Unexpected V3 input manifest revision.' }
if ([long]$inputManifestObject.seed_reserved_for_generation -ne $Seed) { throw 'V3 input manifest seed mismatch.' }
if ((Get-PropertyValue $inputManifestObject 'silhouette_uniqueness_pass') -ne $true) { throw 'V3 input manifest does not report silhouette_uniqueness_pass=true.' }

$seenSilhouettes = @{}
foreach ($name in $ExpectedPoses) {
    $png = Join-Path $PoseDir ($name + '.png')
    $json = Join-Path $SpecDir ($name + '.json')
    if (-not (Test-Path $png -PathType Leaf)) { throw "Missing V3 pose PNG: $png" }
    if (-not (Test-Path $json -PathType Leaf)) { throw "Missing V3 pose spec: $json" }
    ...
    $sil = [string]$spec.binary_silhouette_sha256
    if ([string]::IsNullOrWhiteSpace($sil)) { throw "${name}: missing binary silhouette hash." }
    if ($seenSilhouettes.ContainsKey($sil)) { throw "${name}: duplicate silhouette hash with $($seenSilhouettes[$sil])." }
    $seenSilhouettes[$sil] = $name
}
if ($seenSilhouettes.Count -ne 4) { throw 'V3 requires four unique color-independent silhouettes.' }

New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
New-Item -ItemType Directory -Force -Path $RunDir | Out-Null

if (Test-Path $RunStartedPath -PathType Leaf) { throw "V3 run-start sentinel already exists: $RunStartedPath. Do not rerun automatically." }
if (Test-Path $RunManifestPath -PathType Leaf) { throw "V3 run manifest already exists: $RunManifestPath. Do not rerun." }
if (Test-Path $V3OutputDir -PathType Container) {
    $existing = @(Get-ChildItem -LiteralPath $V3OutputDir -File -Filter '*.png' -ErrorAction SilentlyContinue)
    if ($existing.Count -gt 0) { throw "Existing V3 PNG output(s) found in ${V3OutputDir}. Do not rerun automatically." }
}
...
if ($portBusy) { throw "Port $Port is already in use by a ComfyUI-compatible endpoint." }
...
try {
    ...
    Save-Json ([ordered]@{
        ...
        note = 'V3 one-shot sentinel. Any accepted prompt means do not rerun automatically.'
    }) $RunStartedPath 16

    foreach ($name in $ExpectedPoses) {
        $poseRel = 'refcontrol_poses_v3/' + $name + '.png'
        $prefix = 'flux2_refcontrol_v3/' + $name
        ...
        try {
            ...
            throw "${name}: /prompt submission failed or became transport-ambiguous. Do not rerun automatically. $($_.Exception.Message)"
        }

        $promptId = [string](Get-PropertyValue $submit 'prompt_id')
        if ([string]::IsNullOrWhiteSpace($promptId)) { throw "${name}: no prompt_id returned. Do not rerun automatically." }
        ...
        if ($null -eq $historyEntry) { throw "${name}: timed out waiting for history. Do not resubmit automatically." }
    ...
    if ($records.Count -ne 4) { throw "Expected four completed V3 records, got $($records.Count)." }

    $manifest = [ordered]@{
        spike='FLUX.2 Klein Base 4B FP8 + RefControl Pose'
        revision='v3_walk_distinct_screenspace'
        stage='v3_four_pose_one_shot_complete_visual_qa_pending'
        started_at=$runStarted
        ...
        isolated_change='V3 distinct screen-space COCO-18 gait geometry; V2 correction prompt retained'
        prompt_submissions_expected=4
        prompt_submissions_completed=$records.Count
        retry_policy='exactly one artistic submission per pose; no retry'
        outputs=$records
    ...
    Write-Host ''
    Write-Host 'STEP 8B: PASS' -ForegroundColor Green
    Write-Host 'Exactly four V3 one-shot generations completed.'
    Write-Host "Manifest: $RunManifestPath"
    Write-Host "Outputs:  $V3OutputDir"
    Write-Host 'STOP HERE. Next gate is V2-vs-V3 gait alternation and anatomy QA.' -ForegroundColor Yellow
}
finally {
