...
function Fail([string]$Message) {
    Write-Host "RUNNER42-WAN-W1I: FAIL - $Message" -ForegroundColor Red
    exit 1
}
...
$Executor = Join-Path $ProjectRepoRoot 'tools\wan-animate2-spike\run_w1i_pose_end70_ref15.py'
...
Write-Host ''
Write-Host 'Roguelite Runner 42 - Wan-Animate-2 Base BF16 W1I / W1H aspect-matched branch / pose_end_percent 0.70' -ForegroundColor Cyan
Write-Host "[WORKSPACE] $Workspace" -ForegroundColor Green
Write-Host '[PARENT] Exact completed W1H branch: untouched raw driver, 512x912 aspect-matched canvas, reference_image_strength=1.5.' -ForegroundColor Green
Write-Host '[W1H VERDICT] Major top/right crop failure is resolved; body topology and temporal coherence are materially better than W1G/W1A. Residual destructive blur is concentrated in high-motion phases.' -ForegroundColor Yellow
Write-Host '[ONE VARIABLE FROM W1H] pose_end_percent 1.00 -> 0.70.' -ForegroundColor Yellow
Write-Host '[WHY] Native WanAnimate2ToVideo documentation says motion is mostly established early and explicitly gives ~0.7 as a way to loosen fine detail while retaining choreography.' -ForegroundColor Yellow
Write-Host '[UNCHANGED] Raw driver, 512x912 geometry, Exilada reference/prompt, BF16 stack, 37 frames, 16 fps, 20 steps, CFG 1.0, Euler/simple, shift 5.0, seed 0, pose strength 1.0, pose start 0.0, reference strength 1.5, CLIP pose branch, negative prompt.' -ForegroundColor Green
...
$ExecutorLog = Join-Path $Workspace 'w1i_executor.log'
...
if ($apiRunning) {
    ...
    Write-Host "Stopping previous managed ComfyUI PID $oldPid for a clean W1I launch..." -ForegroundColor Yellow
...
Write-Host 'Submitting W1I: exact W1H branch with pose influence ending at 70% of sampling...' -ForegroundColor Cyan
...
if ($executorExit -ne 0) {
    Write-Host ''
    Write-Host 'W1I executor diagnostics:' -ForegroundColor Red
    ...
    Fail "W1I inference exited with code $executorExit"
}

$Output = Join-Path $Workspace 'w1i_exilada_poseend70_ref15.mp4'
$Manifest = Join-Path $Workspace 'w1i_run_manifest.json'
$Prompt = Join-Path $Workspace 'w1i_api_prompt.json'
foreach ($f in @($Output,$Manifest,$Prompt)) {
    if (-not (Test-Path $f -PathType Leaf)) { Fail "expected W1I proof file missing after inference: $f" }
}

Write-Host ''
Write-Host 'RUNNER42-WAN-W1I: PASS - POSE-END BLUR-ISOLATION DIAGNOSTIC GENERATED' -ForegroundColor Green
...
Write-Host ''
Write-Host 'Next gate: compare W1H vs W1I frame-by-frame. Prefer W1I only if destructive ghosting/blur falls without losing body topology, hair/cloth dynamics or motion adherence.' -ForegroundColor Yellow
