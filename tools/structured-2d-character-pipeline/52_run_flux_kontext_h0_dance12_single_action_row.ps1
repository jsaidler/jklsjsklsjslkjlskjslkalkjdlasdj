...
function Fail([string]$Message) {
    Write-Host "RUNNER52-KONTEXT-H0-DANCE12: FAIL - $Message" -ForegroundColor Red
    exit 1
}
...
$Executor = Join-Path $ProjectRepoRoot 'tools\flux-kontext-spike\run_h0_dance12_single_action_row_structure_lock.py'
$Helper = Join-Path $ProjectRepoRoot 'tools\flux-kontext-spike\run_h0_dance3x4_temporal_rows_structure_lock.py'
...
foreach ($f in @($Python,$MainPy,$Executor,$Helper,$H0Video,$Reference)) {
...
Write-Host ''
Write-Host 'Roguelite Runner 52 - FLUX Kontext / ONE ACTION = ONE 12-FRAME ROW' -ForegroundColor Cyan
Write-Host '[SOURCE] Existing approved H3 Base50 H0 dance_or_gesture video. No new H3 inference.' -ForegroundColor Green
Write-Host '[FINAL LAYOUT] 12 columns x 1 row, 192x192 cells, 2304x192 total.' -ForegroundColor Green
Write-Host '[PROCESSING ONLY] Three 2x2 Kontext tiles of four ordered frames each; they are NOT final rows.' -ForegroundColor Green
Write-Host '[STRUCTURE LOCK] denoise 0.45; preserve mature adult anatomy and exact character identity.' -ForegroundColor Yellow
Write-Host '[ART LOCK] Heavy Metal / Conan / Red Sonja / Frazetta / Julie Bell remains mandatory.' -ForegroundColor Yellow
Write-Host ''
...
$apiRunning = $false
try { $null = Invoke-RestMethod -Uri "$Base/system_stats" -TimeoutSec 2; $apiRunning = $true } catch {}
if ($apiRunning) { Fail "port $Port is already serving an unmanaged process. Stop it or choose another port." }

$launchArgs = @('-s',$MainPy,'--windows-standalone-build','--listen','127.0.0.1','--port',"$Port",'--disable-auto-launch')
Write-Host 'Starting isolated pinned ComfyUI v0.34.0 for Runner52...' -ForegroundColor Cyan
...
for ($i=0; $i -lt 300; $i++) {
    try { $null = Invoke-RestMethod -Uri "$Base/system_stats" -TimeoutSec 2; $ready = $true; break }
        ...
        if ($process.HasExited) {
            ...
            Fail "Kontext ComfyUI exited before Runner52 submission with code $($process.ExitCode)"
        }
...
Write-Host 'Submitting Runner52 single-action 1x12 proof...' -ForegroundColor Cyan
...
if ($executorExit -ne 0) {
    Write-Host ''
    Write-Host 'Runner52 executor diagnostics:' -ForegroundColor Red
    ...
    Fail "Runner52 executor exited with code $executorExit. Classify the failure layer before changing model/settings."
}

$Expected = @(
    (Join-Path $Workspace 'h0_dance12_source_action_strip.png'),
    (Join-Path $Workspace 'h0_dance12_action_selection_manifest.json'),
    (Join-Path $Workspace 'h0_dance12_pixelart_sheet_opaque.png'),
    (Join-Path $Workspace 'h0_dance12_pixelart_sheet_rgba.png'),
    (Join-Path $Workspace 'h0_dance12_preview.gif'),
...
foreach ($f in $Expected) {
    if (-not (Test-Path $f -PathType Leaf)) {
        ...
        Fail "expected Runner52 output missing: $f"
    }
}
...
Write-Host ''
Write-Host 'RUNNER52-KONTEXT-H0-DANCE12: PASS - THREE INTERNAL CHUNK INFERENCES COMPLETE / VISUAL VERDICT PENDING' -ForegroundColor Green
Write-Host "Source action strip: $(Join-Path $Workspace 'h0_dance12_source_action_strip.png')" -ForegroundColor Cyan
Write-Host "Opaque 1x12 sheet: $(Join-Path $Workspace 'h0_dance12_pixelart_sheet_opaque.png')" -ForegroundColor Cyan
Write-Host "RGBA 1x12 sheet:   $(Join-Path $Workspace 'h0_dance12_pixelart_sheet_rgba.png')" -ForegroundColor Cyan
Write-Host "Action preview GIF: $(Join-Path $Workspace 'h0_dance12_preview.gif')" -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'Review one action as one row: temporal continuity left-to-right, adult anatomy, pose fidelity, cross-chunk consistency, pixel-art quality, alpha and 1980s art direction.' -ForegroundColor Yellow
