...
def bootstrap_mpfb(mpfb_root: Path, user_root: Path):
    ...
    print("G3S_B3_MPFB_BOOTSTRAP=PASS")
    print(f"G3S_B3_MPFB_ROOT={mpfb_root}")
    print(f"G3S_B3_MPFB_USER_ROOT={user_root}")
...
def main():
    ...
    if not target_script.exists():
        raise RuntimeError(f"G3S-B3 target script not found: {target_script}")

    bootstrap_mpfb(mpfb_root, user_root)
    namespace = runpy.run_path(str(target_script), run_name="g3s_b3_target")
    target_main = namespace.get("main")
    if target_main is None or not callable(target_main):
        raise RuntimeError("G3S-B3 target did not expose callable main()")
    target_main()
