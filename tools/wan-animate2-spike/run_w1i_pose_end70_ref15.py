...
def fail(message, code=2):
    print(f"W1I: FAIL - {message}", file=sys.stderr, flush=True)
...
def main():
    ...
    if int(parent_manifest.get("width", 0)) != EXPECTED_WIDTH or int(parent_manifest.get("height", 0)) != EXPECTED_HEIGHT:
        fail(f"W1I must branch from W1H 512x912 geometry, found {parent_manifest.get('width')}x{parent_manifest.get('height')}")
    if float(parent_manifest.get("reference_image_strength", -1)) != EXPECTED_REFERENCE_STRENGTH:
        fail("W1I must preserve W1H reference_image_strength=1.5")
    if float(parent_manifest.get("pose_strength", -1)) != EXPECTED_POSE_STRENGTH:
        fail("W1I must preserve W1H pose_strength=1.0")
    ...
    before_pose_end = float(inputs.get("pose_end_percent", -1))
    if before_pose_end != 1.0:
        fail(f"W1H pose_end_percent is not 1.0: {before_pose_end}")

    inputs["pose_end_percent"] = TARGET_POSE_END
    ...
    new_prefix = "roguelite_w1i/wan_animate2_bf16_exilada_aspectmatched_ref15_poseend70"
    ...
    prompt_out = os.path.join(workspace, "w1i_api_prompt.json")
    with open(prompt_out, "w", encoding="utf-8") as fh:
        json.dump(prompt, fh, ensure_ascii=False, indent=2)

    print(
        "W1I: submitting exact W1H geometry/identity branch with one native motion-conditioning change only: "
        f"pose_end_percent {before_pose_end:.2f} -> {TARGET_POSE_END:.2f}.",
        flush=True,
    )
    print(
        "W1I: purpose = reduce destructive high-motion ghosting after choreography is established, while preserving raw driver, "
        "512x912 aspect-matched framing, pose_strength=1.0 and reference_image_strength=1.5.",
        flush=True,
    )
    ...
    print(f"W1I: prompt_id={prompt_id}", flush=True)
    ...
    while time.time() < deadline:
        ...
        now = time.time()
        if now >= next_report:
            print(f"W1I: inference running for {int(now - started)} seconds...", flush=True)
            next_report = now + 60
        time.sleep(5)
    else:
    ...
    canonical_output = os.path.join(workspace, "w1i_exilada_poseend70_ref15.mp4")
    shutil.copy2(output_path, canonical_output)

    manifest = dict(parent_manifest)
    manifest.update({
        "gate": "WAN_ANIMATE2_W1I_BF16_EXILADA_ASPECTMATCHED_REF15_POSEEND70",
        "status": "INFERENCE_COMPLETE",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "parent_w1h_prompt_id": parent_manifest.get("prompt_id"),
        "pose_end_percent": TARGET_POSE_END,
        "changed_from_w1h": ["pose_end_percent_1.0_to_0.70", "output_prefix"],
        "unchanged_from_w1h": [
            ...
            "scheduler", "shift", "seed", "pose_strength", "pose_start_percent", "reference_image_strength",
            "positive_pose", "clip_vision_output_pose"
        ],
        "hypothesis": "W1H fixed the dominant framing/canvas mismatch and materially reduced destructive topology loss, but residual high-motion blur remains. Native WanAnimate2ToVideo documentation states motion is mostly established early and gives pose_end_percent=0.7 as an example that can loosen fine detail while keeping choreography. W1I therefore changes only pose_end_percent from 1.0 to 0.70 on the exact W1H branch.",
    ...
    manifest_path = os.path.join(workspace, "w1i_run_manifest.json")
    with open(manifest_path, "w", encoding="utf-8") as fh:
        json.dump(manifest, fh, ensure_ascii=False, indent=2)

    print(f"W1I: PASS - generated {canonical_output}", flush=True)
    print(f"W1I: manifest {manifest_path}", flush=True)
    print(f"W1I: elapsed {elapsed} seconds", flush=True)
