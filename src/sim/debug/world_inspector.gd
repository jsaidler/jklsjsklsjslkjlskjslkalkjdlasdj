...
static func event_count(world: WorldState, event_type: String) -> int:
    ...
    for event in world.events:
        if event["type"] == event_type: count += 1
    return count
...
static func first_knowledge_delivery_for(world: WorldState, target_id: String, source_event_id: int) -> Dictionary:
    for event in world.events:
        if event["type"] != "knowledge_delivered": continue
        var payload: Dictionary = event["payload"]
        if payload["target_id"] == target_id and int(payload["source_event_id"]) == source_event_id: return event
    return {}
...
static func last_decision(world: WorldState, subject_id: String, decision_type: String) -> Dictionary:
    for index in range(world.decision_log.size() - 1, -1, -1):
        ...
        if decision["subject_id"] == subject_id and decision["decision_type"] == decision_type: return decision
    return {}

static func explain_decision(decision: Dictionary) -> String:
    if decision.is_empty(): return "No decision found."
    var score_parts: Array[String] = []
    var scores: Dictionary = decision["scores"]
    var keys := scores.keys()
    keys.sort()
    for key in keys: score_parts.append("%s=%0.3f" % [String(key), float(scores[key])])
...
static func signature(world: WorldState) -> String:
    ...
    for event in world.events: sequence.append("%d:%s" % [int(event["day"]), String(event["type"])])
